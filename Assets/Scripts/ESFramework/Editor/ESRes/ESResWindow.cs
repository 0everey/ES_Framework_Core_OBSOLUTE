...
namespace ES
{
    ...
    public class ESResWindow : ESWindowBase_Abstract<ESResWindow>
    {
        ...
        public class PartPageClass_AssetBundle : ESWindowPageBase
        {
            ...
            [LabelText("更改AB打包模式"),ShowInInspector]public ABPackType packType { get => EditorMaster.Instance.abPackType; set => EditorMaster.Instance.abPackType = value; }
            [InfoBox("",Message = "@GetCodeInfo()",InfoMessageType = InfoMessageType.Warning)]
            [LabelText("打AB包时，自动辅助代码生成格式"), PropertyOrder(-3), ShowInInspector,InlineButton("Handle_CodeGenAssetConstName", "手动生成常量名代码")] public ABForAutoCodeGen codeType { get => EditorMaster.Instance.abFoeAutoCodeGen; set => EditorMaster.Instance.abFoeAutoCodeGen = value; }
            string GetCodeInfo()
            {
                switch (codeType)
                {
                    case ABForAutoCodeGen.NoneCode: return "不生成代码";
                    case ABForAutoCodeGen.CodeAsOriginal: return "生成代码(完全按源文件名)";
                    case ABForAutoCodeGen.CodeAsLower: return "生成代码(源文件名转小写)";
                    case ABForAutoCodeGen.CodeAsUpper: return "生成代码(源文件名转大写)";
            ...
            [FolderPath, LabelText("生成路径"), ShowInInspector]
            public string genarateFolder { get => EditorMaster.Instance.genarateFolder; set => EditorMaster.Instance.genarateFolder = value; }
            ...
            private void Handle_BuildAB()
            {
                ...
                {
                    BuildPipeline.BuildAssetBundles(genarateFolder,assetBundleOptions: BuildAssetBundleOptions.None,targetPlatform: BuildTarget.StandaloneWindows );
                }
                else
                {
            ...
            public string selectBundleName { get => EditorMaster.Instance.ABName; set => EditorMaster.Instance.ABName = value; }
            [TitleGroup("AB包查询与标记")]
            [LabelText("标记模式"), ShowInInspector]
            public ABMaskType maskType { get => EditorMaster.Instance.abMaskType; set => EditorMaster.Instance.abMaskType = value; }
