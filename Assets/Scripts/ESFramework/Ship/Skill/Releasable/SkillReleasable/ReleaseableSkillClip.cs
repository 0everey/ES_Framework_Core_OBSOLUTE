...
namespace ES
{
    ...
    public class ReleasableSkillsSequence
    {
        ...
        [VerticalGroup("总数据/数据"), PropertyOrder(-2)][LabelText("总时间")] public float skillDuration = 1f;
        [Space(5)]
        ...
        [FoldoutGroup("总数据/数据/数据细节"),LabelText("前后摇曲线(<0.5视为)")] public AnimationCurve skillActiveCurve = AnimationCurve.Constant(0.1f,0.9f,1);
        [FoldoutGroup("总数据/数据/数据细节"), LabelText("技能绑定的ES状态配置")] public StateDataInfo bindingStateInfo; 
        [FoldoutGroup("总数据/数据/数据细节"), LabelText("词条")] public List<string> skillTags = new List<string>();
        [FoldoutGroup("总数据/数据/数据细节"), LabelText("冷却时间"), SerializeReference] public IPointerForFloat_Only coolDown = new PointerForFloat_Random30() { float_range = new Vector2(5,15) };


        [HorizontalGroup("分栏目",width: 0.9f)]
        [ShowInInspector,PropertyOrder(-1),BoxGroup("分栏目/当前", CenterLabel = true, LabelText = "【编辑片段】"),DisplayAsString(fontSize:25), HideLabel, GUIColor("GetShowStartEditColor")]
        public string _ss=> GetShowStartEditName();
        [BoxGroup("分栏目/当前"),HideLabel,InlineProperty,
            GUIColor("@KeyValueMatchingUtility.ColorSelector.ColorForBinding")
            ]
        ...
        private Color GetShowStartEditColor()
        {
            ...
            return KeyValueMatchingUtility.ColorSelector.ColorForCaster;
        }
    ...
    {
        [HorizontalGroup("总数据")]
        [LabelText("触发时机(秒)")]public float triggerAtTime=0.25f;
        [LabelText("命名")] public string name = "新技能片段";


        [HorizontalGroup("分栏目",width:0.25f)]
        [BoxGroup("分栏目/绑定状态与动画",CenterLabel =true,LabelText ="【绑定源】") ,GUIColor("@KeyValueMatchingUtility.ColorSelector.ColorForBinding")]
        
        [LabelText("绑定原生状态")]
        public string st = "技能/技能1";

        [BoxGroup("分栏目/绑定状态与动画")]
        [LabelText("过渡时间(归一化)")]
        ...
        [HorizontalGroup("分栏目", width: 0.38f, MarginLeft = 25)]
        [BoxGroup("分栏目/筛选角色", CenterLabel = true, LabelText = "【筛选对象】"), GUIColor("@KeyValueMatchingUtility.ColorSelector.ColorForSearch")]
        ...
        [Space(20)]
        [BoxGroup("分栏目/筛选角色")]public IPointerForSomeEntity_Only Selector = new SomeEntitySelectorFromSelf();





        [HorizontalGroup("分栏目", width:0.28f, MarginLeft = 15)]
        [BoxGroup("分栏目/应用效果", CenterLabel = true, LabelText = "【作用效果】"), GUIColor("@KeyValueMatchingUtility.ColorSelector.ColorForApply")]
        [LabelText("对筛选目标逐个执行")]
